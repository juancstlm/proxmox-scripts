...
| --------------- | ------------------------ | --- |
| `setup-arr-vm.sh` | Proxmox **host**       | Picks next VMID, downloads the Debian 12 cloud image if missing, creates the VM with cloud-init (static IP derived from VMID — `10.8.1.<VMID>/24` — on Apps VLAN, SSH key seeded), prints the VM's MAC and pauses so you can pin it in UniFi, then SSHes in and runs `provision.sh`. |
...
The script pauses after creating the VM to remind you to add the **per-MAC
override** (UniFi assigns the VLAN by MAC because `vmbr0` isn't VLAN-aware
on this PVE host).
...
sudo docker logs gluetun | grep 'Port forwarded'
```

The forwarded port from ProtonVPN's NAT-PMP shows up in the gluetun logs
after the tunnel comes up. Paste it into qBittorrent → Connection → Port,
or wire up `gluetun-qbittorrent-port-manager` as a sidecar (see Arr
Stack.md).
